...
interface AtomicPattern {
   ...
   *
   *  High values = dense subdivision (many steps per beat):
   *    4 = sixteenth notes (standard)
   *    3 = triplets (12/8 feel)
   *    6 = sextuplets
   *    2 = eighth notes
   *    1 = quarter notes
   *
   *  Fractional values = sparse subdivision (fewer steps than beats):
   *    0.5   = one step every 2 beats (half notes)
   *    0.25  = one step every 4 beats (whole notes)
   *    1/16  = one step every 16 beats
   *    1/64  = one step every 64 beats (gamelan kenong)
   *    1/256 = one step per 256-beat cycle (gamelan gong ageng)
   *
   *  The step duration formula:
   *    stepDuration = 60 / tempo / stepsPerBeat
   *
   *  This single field handles everything from 32nd-note hi-hats
   *  (stepsPerBeat: 8) to a gong that strikes once per 256-beat
   *  cycle (stepsPerBeat: 1/256). Each track advances at its own
   *  rate, so colotomic layering, polyrhythm, and mixed subdivisions
   *  all fall out naturally.
   *
   *  Gamelan colotomic example at 60 BPM (1 beat = 1 second):
   *    Peking:      stepsPerBeat: 4,     stepDuration = 0.25s
   *    Ketuk:       stepsPerBeat: 1/16,  stepDuration = 16s
   *    Kempul:      stepsPerBeat: 1/32,  stepDuration = 32s
   *    Kenong:      stepsPerBeat: 1/64,  stepDuration = 64s
   *    Gong ageng:  stepsPerBeat: 1/256, stepDuration = 256s (~4 min)
   */
  stepsPerBeat: number;
...
**Gamelan colotomic layers (different subdivisions per track):**

Instead of a 256-step grid with mostly rests, each instrument gets only
the steps it actually plays — `stepsPerBeat` controls density:

```typescript
// All five tracks in a stack, sharing tempo = 60 BPM.
// Each track has a different stepsPerBeat, so each advances at its own rate.

// Peking (fastest — plays every 16th note)
{ id: "peking", instrument: "peking",
  stepCount: 16, stepsPerBeat: 4,       // 16 steps per 4-beat bar
  steps: [/* 16th-note elaboration pattern */] }

// Ketuk (strikes every 4 beats)
{ id: "ketuk", instrument: "ketuk",
  stepCount: 4, stepsPerBeat: 0.25,     // 1 step per 4 beats
  steps: [null, { velocity: 80 }, null, { velocity: 80 }] }

// Kempul (strikes every 8 beats)
{ id: "kempul", instrument: "kempul",
  stepCount: 4, stepsPerBeat: 0.125,    // 1 step per 8 beats
  steps: [null, { velocity: 90 }, null, { velocity: 90 }] }

// Kenong (strikes every 16 beats)
{ id: "kenong", instrument: "kenong",
  stepCount: 4, stepsPerBeat: 0.0625,   // 1 step per 16 beats
  steps: [null, null, null, { velocity: 100 }] }

// Gong ageng (strikes once per 64-beat cycle)
{ id: "gong", instrument: "gong-ageng",
  stepCount: 1, stepsPerBeat: 1/64,     // 1 step per 64 beats
  isReference: true,                     // the gong IS the timeline
  steps: [{ velocity: 127 }] }
```

At 60 BPM:
- Peking step = 0.25s (16th notes, fast metallophone elaboration)
- Ketuk step = 4s (marks every 4th beat)
- Kempul step = 8s (marks structural divisions)
- Kenong step = 16s (marks major sections)
- Gong step = 64s (one strike per entire cycle — ~1 minute)

The beauty: each track has only 1–16 steps to edit. No 256-step grids
with 255 empty cells. The `stepsPerBeat` field does all the work.
...
| **Gamelan colotomic** | Gong (1/cycle), kenong (4/cycle), ketuk (16/cycle) | Stack with small stepCounts + fractional stepsPerBeat (e.g., kenong: 4 steps at 1/64 stepsPerBeat) |
